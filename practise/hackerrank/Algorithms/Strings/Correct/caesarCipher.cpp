...
string caesarCipher(string s, int k) {
    ...
    // shift alphabets by k or by a shift factor, for k > 26
    int alpha_size = lower_case.size();
    int factor = k/alpha_size;
    int shift = k-(factor*alpha_size);
    if(k > alpha_size) {
        std::rotate(lower_case.begin(), lower_case.begin()+shift, lower_case.end());
        std::rotate(upper_case.begin(), upper_case.begin()+shift, upper_case.end());
    }
    else {
